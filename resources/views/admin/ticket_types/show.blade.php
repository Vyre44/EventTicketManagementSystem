...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h1 class="h4 mb-0">TicketType #{{ $ticketType->id }}</h1>
    <div class="d-flex gap-2">
        <a href="{{ route('admin.ticket-types.edit', $ticketType) }}" class="btn btn-primary btn-sm">Duzenle</a>
...
<div class="card shadow-sm mb-4">
    <div class="card-body d-flex justify-content-between align-items-start">
        <div>
            <div class="text-muted">TicketType</div>
            <div class="h5 mb-0">{{ $ticketType->name }}</div>
        </div>
        <span class="badge {{ $ticketType->is_active ? 'bg-success' : 'bg-secondary' }}">{{ $ticketType->is_active ? 'Evet' : 'Hayir' }}</span>
    </div>
</div>

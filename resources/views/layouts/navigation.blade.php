    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            ...
            <div class="mt-3 space-y-1">
                <!-- Profile link (responsive-nav-link component kaldırıldı) -->
                <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-base text-gray-700 hover:bg-gray-100">{{ __('Profile') }}</a>

                <!-- Authentication -->
                <form method="POST" action="{{ route('logout') }}">
                    @csrf

                    <!-- Logout link (responsive-nav-link component kaldırıldı) -->
                    <a href="{{ route('logout') }}" onclick="event.preventDefault(); this.closest('form').submit();" class="block px-4 py-2 text-base text-gray-700 hover:bg-gray-100">{{ __('Log Out') }}</a>
                </form>
            </div>
        </div>
    </div>

...
<form method="POST" action="{{ route('admin.ticket-types.update', $ticketType) }}">
    ...
    <input name="name" placeholder="Ad" value="{{ old('name', $ticketType->name) }}">
    <input name="price" type="number" step="0.01" placeholder="Fiyat" value="{{ old('price', $ticketType->price) }}">
    <input name="quota" type="number" placeholder="Kota" value="{{ old('quota', $ticketType->quota) }}">
